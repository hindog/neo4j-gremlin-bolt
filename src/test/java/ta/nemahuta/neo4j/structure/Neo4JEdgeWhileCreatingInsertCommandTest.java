...
public class Neo4JEdgeWhileCreatingInsertCommandTest {
    ...
    public void givenNoIdGenerationProviderShouldCreateInsertCommand() {
        ...
        Mockito.when(edgeIdProvider.convert(Mockito.any())).thenAnswer(invocation -> 3L);
        ...
        // assert
        Assert.assertNull("Failed retrieveId node identifier", edge.id());
        ...
        // assert
        Assert.assertNotNull("Failed retrieveId node identifier", edge.id());
    }
    ...
    public void givenIdGenerationProviderShouldCreateInsertCommand() {
        ...
        Mockito.when(graph.tx()).thenAnswer(invocation -> transaction);
        Mockito.when(outVertex.matchPattern(Mockito.any())).thenAnswer(invocation -> "(o)");
        Mockito.when(outVertex.matchPredicate(Mockito.any(), Mockito.any())).thenAnswer(invocation -> "o.whereId = {oid}");
        Mockito.when(outVertex.id()).thenAnswer(invocation -> 1L);
        Mockito.when(outVertex.matchStatement(Mockito.anyString(), Mockito.anyString())).thenAnswer(invocation -> "MATCH (o) WHERE o.whereId = {oid}");
        Mockito.when(inVertex.matchPattern(Mockito.any())).thenAnswer(invocation -> "(i)");
        Mockito.when(inVertex.matchPredicate(Mockito.any(), Mockito.any())).thenAnswer(invocation -> "i.whereId = {iid}");
        Mockito.when(inVertex.id()).thenAnswer(invocation -> 2L);
        Mockito.when(inVertex.matchStatement(Mockito.anyString(), Mockito.anyString())).thenAnswer(invocation -> "MATCH (i) WHERE i.whereId = {iid}");
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 3L);
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        ...
        Assert.assertNotNull("Failed to create insert command", command);
        Assert.assertNotNull("Failed to create insert command statement", command.getStatement());
        Assert.assertEquals("Invalid insert command statement", command.getStatement().text(), "MATCH (o) WHERE o.whereId = {oid} MATCH (i) WHERE i.whereId = {iid} CREATE (o)-[:`L1`{ep}]->(i)");
        Assert.assertEquals("Invalid insert command statement", command.getStatement().parameters(), Values.parameters("oid", 1L, "iid", 2L, "ep", Collections.singletonMap("whereId", 3L)));
        Assert.assertNotNull("Failed to create insert command callback", command.getCallback());
        // invoke callback
        command.getCallback().accept(statementResult);
        // assert
