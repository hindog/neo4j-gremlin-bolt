...
public class Neo4JVertexWhileGettingInOutEdgesTest {
    ...
    @Mock
    private Neo4JElementIdAdapter vertexIdProvider;

    @Mock
    private Neo4JElementIdAdapter edgeIdProvider;
    ...
    public void givenNoLabelsShouldGetEdges() {
        ...
        Mockito.when(node.get(Mockito.eq("whereId"))).thenAnswer(invocation -> Values.value(1L));
        ...
        Mockito.when(vertexIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 1L);
        Mockito.when(vertexIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(vertexIdProvider.matchPredicateOperand(Mockito.any())).thenAnswer(invocation -> "n.whereId");
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 2L);
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.matchPredicateOperand(Mockito.any())).thenAnswer(invocation -> "r.whereId");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r]-(m) WHERE n.whereId = {whereId} RETURN n, r, m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        ...
        // assert
        Assert.assertNotNull("Failed to retrieveId edge iterator", edges);
        Assert.assertTrue("Edges iterator is empty", edges.hasNext());
        Assert.assertNotNull("Failed to retrieveId edge", edges.next());
        Assert.assertTrue("Edges iterator is empty", edges.hasNext());
        Assert.assertNotNull("Failed to retrieveId edge", edges.next());
    }
    ...
    public void givenNoLabelsShouldGetDatabaseAndTransientEdges() {
        ...
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN n, r, m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
    ...
    public void givenLabelShouldGetDatabaseAndTransientEdges() {
        ...
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r:`EL`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN n, r, m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addOutEdge(edge2);
    ...
    public void givenLabelsShouldGetDatabaseAndTransientEdges() {
        ...
        Mockito.when(edge4.id()).thenAnswer(invocation -> 400L);
        Mockito.when(edge4.label()).thenAnswer(invocation -> "EL2");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r:`EL2`|:`EL1`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN n, r, m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addOutEdge(edge2);
    ...
    public void givenLabelShouldGetDatabaseEdges() {
        ...
        Mockito.when(edge4.id()).thenAnswer(invocation -> 400L);
        Mockito.when(edge4.label()).thenAnswer(invocation -> "EL2");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r:`EL1`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN n, r, m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addOutEdge(edge2);
    ...
    public void givenLabelShouldGetTransientEdges() {
        ...
        Mockito.when(edge4.id()).thenAnswer(invocation -> 400L);
        Mockito.when(edge4.label()).thenAnswer(invocation -> "EL2");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)-[r:`EL2`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN n, r, m", parameters)))).thenAnswer(invocation -> statementResult);
    ...
    public void givenNoLabelsAndPartitionMatchPatternShouldGetEdges() {
        ...
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.matchPredicateOperand(Mockito.any())).thenAnswer(invocation -> "r.whereId");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`P1`:`P2`:`l1`)-[r]-(m:`P1`:`P2`) WHERE n.whereId = {whereId} RETURN n, r, m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        // act
    ...
    public void givenNoLabelsAndPartitionMatchPredicateShouldGetEdges() {
        ...
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.matchPredicateOperand(Mockito.any())).thenAnswer(invocation -> "r.whereId");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`P1`:`l1`)-[r]-(m) WHERE n.whereId = {whereId} AND (m:`P1` OR m:`P2`) RETURN n, r, m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.edges(Mockito.eq(statementResult))).thenAnswer(invocation -> Arrays.asList(edge1, edge3).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        // act
