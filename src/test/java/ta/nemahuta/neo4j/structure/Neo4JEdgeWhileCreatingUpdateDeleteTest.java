...
public class Neo4JEdgeWhileCreatingUpdateDeleteTest {
    ...
    public void givenNewPropertyShouldCreateUpdateCommand() {
        ...
        Mockito.when(relationship.get(Mockito.eq("key1"))).thenAnswer(invocation -> Values.value("value1"));
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 3L);
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> null);
        ...
        Assert.assertEquals("Invalid insert command statement", command.getStatement().text(), "MATCH (o) WHERE ID(o) = {oid} MATCH (i) WHERE ID(i) = {iid} MATCH (o)-[r:`label`]->(i) WHERE ID(r) = {whereId} SET r = {rp}");
        Assert.assertEquals("Invalid insert command statement", command.getStatement().parameters(), Values.parameters("oid", 1L, "iid", 2L, "whereId", 3L, "rp", Collections.singletonMap("key1", "value1")));
    ...
    public void givenRemovedPropertyShouldCreateUpdateCommand() {
        ...
        Assert.assertNotNull("Failed to create insert command statement", command.getStatement());
        Assert.assertEquals("Invalid insert command statement", command.getStatement().text(), "MATCH (o) WHERE ID(o) = {oid} MATCH (i) WHERE ID(i) = {iid} MATCH (o)-[r:`label`]->(i) WHERE ID(r) = {whereId} SET r = {rp}");
        Assert.assertEquals("Invalid insert command statement", command.getStatement().parameters(), Values.parameters("oid", 1L, "iid", 2L, "whereId", 3L, "rp", Collections.singletonMap("key1", null)));
        Assert.assertNotNull("Failed to create insert command callback", command.getCallback());
        // invoke callback
        command.getCallback().accept(statementResult);
        // assert
