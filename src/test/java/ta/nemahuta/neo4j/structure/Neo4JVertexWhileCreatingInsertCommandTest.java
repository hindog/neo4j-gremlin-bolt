...
public class Neo4JVertexWhileCreatingInsertCommandTest {
    ...
    public void givenNoIdGenerationProviderShouldCreateInsertCommand() {
        ...
        Mockito.when(vertexIdProvider.convert(Mockito.any())).thenAnswer(invocation -> 1L);
        ...
        // assert
        Assert.assertNull("Failed retrieveId node identifier", vertex.id());
        ...
        // assert
        Assert.assertNotNull("Failed retrieveId node identifier", vertex.id());
    }
    ...
    public void givenIdGenerationProviderShouldCreateInsertCommand() {
        ...
        Assert.assertEquals("Invalid insert command statement", command.getStatement().parameters(), Values.parameters("vp", Collections.singletonMap("whereId", 1L)));
        Assert.assertNotNull("Failed to create insert command callback", command.getCallback());
    }
}
