...
public class Neo4JVertexWhileGettingInVerticesTest {
    ...
    public void givenNoLabelsShouldGetVertices() {
        ...
        Mockito.when(vertexIdProvider.matchPredicateOperand(Mockito.anyString())).thenAnswer(invocation -> "n.whereId");
        ...
        Mockito.when(edgeIdProvider.matchPredicateOperand(Mockito.anyString())).thenAnswer(invocation -> "r.whereId");
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r]-(m) WHERE n.whereId = {whereId} RETURN m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        ...
        // assert
        Assert.assertNotNull("Failed to retrieveId vertex iterator", vertices);
        Assert.assertTrue("Vertex iterator is empty", vertices.hasNext());
        Assert.assertNotNull("Failed to retrieveId vertex", vertices.next());
    }
    ...
    public void givenNoLabelsShouldGetDatabaseAndTransientVertices() {
        ...
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
    ...
    public void givenLabelShouldGetDatabaseAndTransientVertices() {
        ...
        Mockito.when(edge2.outVertex()).thenAnswer(invocation -> vertex2);
        Mockito.when(edge2.id()).thenAnswer(invocation -> 2000L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r:`EL`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addInEdge(edge2);
    ...
    public void givenLabelsShouldGetDatabaseAndTransientVertices() {
        ...
        Mockito.when(edge2.outVertex()).thenAnswer(invocation -> vertex2);
        Mockito.when(edge2.id()).thenAnswer(invocation -> 2000L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r:`EL2`|:`EL1`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addInEdge(edge2);
    ...
    public void givenLabelShouldGetDatabaseVertices() {
        ...
        Mockito.when(edge2.outVertex()).thenAnswer(invocation -> vertex2);
        Mockito.when(edge2.id()).thenAnswer(invocation -> 2000L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r:`EL1`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN m", parameters)))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        vertex.addInEdge(edge2);
    ...
    public void givenLabelShouldGetTransientVertices() {
        ...
        Mockito.when(edgeIdProvider.matchPredicateOperand(Mockito.anyString())).thenAnswer(invocation -> "r.whereId");
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 1L);
        Mockito.when(vertex1.id()).thenAnswer(invocation -> 100L);
        Mockito.when(vertex2.id()).thenAnswer(invocation -> 200L);
        Mockito.when(edge2.label()).thenAnswer(invocation -> "EL2");
        Mockito.when(edge2.outVertex()).thenAnswer(invocation -> vertex2);
        Mockito.when(edge2.id()).thenAnswer(invocation -> 2000L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`l1`)<-[r:`EL2`]-(m) WHERE n.whereId = {whereId} AND NOT r.whereId IN {ids} RETURN m", parameters)))).thenAnswer(invocation -> statementResult);
    ...
    public void givenNoLabelsAndPartitionMatchPatternShouldGetVertices() {
        ...
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 1L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`P1`:`P2`:`l1`)<-[r]-(m:`P1`:`P2`) WHERE n.whereId = {whereId} RETURN m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        // act
    ...
    public void givenNoLabelsAndPartitionMatchPredicateShouldGetVertices() {
        ...
        Mockito.when(edgeIdProvider.propertyName()).thenAnswer(invocation -> "whereId");
        Mockito.when(edgeIdProvider.retrieveId(Mockito.any())).thenAnswer(invocation -> 1L);
        Mockito.when(session.executeStatement(Mockito.eq(new Statement("MATCH (n:`P1`:`l1`)<-[r]-(m) WHERE n.whereId = {whereId} AND (m:`P1` OR m:`P2`) RETURN m", Collections.singletonMap("whereId", 1L))))).thenAnswer(invocation -> statementResult);
        Mockito.when(session.vertices(Mockito.eq(statementResult))).thenAnswer(invocation -> Collections.singleton(vertex1).stream());
        Mockito.when(statementResult.consume()).thenAnswer(invocation -> resultSummary);
        Neo4JVertex vertex = new Neo4JVertex(graph, session, vertexIdProvider, edgeIdProvider, node);
        // act
