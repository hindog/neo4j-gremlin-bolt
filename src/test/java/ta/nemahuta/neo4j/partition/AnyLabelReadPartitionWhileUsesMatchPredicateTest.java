...
public class AnyLabelReadPartitionWhileUsesMatchPredicateTest {
    ...
    public void givenPartitionWithOneLabelShouldReturnFalse() {
        ...
        // assert
        Assert.assertFalse("Invalid response for AnyLabelGraphPartition with one label", result);
    }

    @Test
    public void givenPartitionWithMultipleLabelsShouldReturnTrue() {
        // arrange
        Neo4JGraphPartition partition = new AnyLabelGraphPartition("l1", "l2");
        // act
        boolean result = partition.usesMatchPredicate();
        // assert
        Assert.assertTrue("Invalid response for AnyLabelGraphPartition with multiple orLabelsAnd", result);
    }
}
